...
; administrative step to dereference Ξ when ι empty
; local Ξ
      ...
      (match s
        ...
        ((ko (cons (letreck a e ρ) ι) κ v σ Ξ)
         (let ((σ* (store-alloc σ a v)))
           (set (ev e ρ σ* ι κ Ξ))))
        ((ko '() τ v σ Ξ)
         (let ((ικs (stack-lookup Ξ τ)))
           (list->set (set-map ικs (lambda (ικ) (ko (car ικ) (cdr ικ) v σ Ξ))))))
...
(time
 ...
 (conc-eval rotate)
 (type-eval rotate)
 ) ; cpu time: 282500 real time: 282440 gc time: 197
