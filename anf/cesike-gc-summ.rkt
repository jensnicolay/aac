...
(struct ko (ι κ v σ Ξ m) #:transparent)
...
(define (make-step α γ ⊥ ⊔ alloc true? false?)
  ...
  (define (touches d)
    (if (set? d)
        ...
        (match d
          ((ev _ ρ _ ι κ Ξ _) (set-union (env-addresses ρ) (apply set-union (set-map (stack-frames ι κ Ξ) touches))))
          ((ko ι κ v _ Ξ _) (set-union (apply set-union (set-map (stack-frames ι κ Ξ) touches)) (touches v)))
  ...
  (lambda (s)
    (match s
      ((ev (? ae? ae) ρ σ ι κ Ξ m)
       (let ((v (eval-atom ae ρ σ)))
         (set (ko ι κ v σ Ξ m))))
      ((ev `(if ,ae ,e1 ,e2) ρ σ ι κ Ξ m)
       (let ((v (eval-atom ae ρ σ)))
         (set-union (if (true? v)
                        (set (ev e1 ρ σ ι κ Ξ m))
                        (set))
                    (if (false? v)
                        (set (ev e2 ρ σ ι κ Ξ m))
                        (set)))))
      ...
      ((ev `(set! ,x ,ae) ρ σ ι κ Ξ m)
       (let* ((v (eval-atom ae ρ σ))
              (a (env-lookup ρ x))
              (σ* (store-update σ a v)))
         (set (ko ι κ v σ* Ξ m))))
      ((ev `(,rator . ,rands) ρ σ ι κ Ξ m)
                      ...
                      (let ((τ (ctx w rvs Γσ)))
                        (if (hash-has-key? m τ)
                            ...
                            (let loop ((x x) (vs (reverse rvs)) (ρ* ρ**) (σ* Γσ))
                              (match x
                                ('()
                                 (let ((Ξ* (stack-alloc Ξ τ (cons ι κ))))
                                   (set-union states (set (ev e0 ρ* σ* '() τ Ξ* m)))))
                                ((cons x xs)
                                 (if (null? vs)
                                     (set)
                                     (let ((a (alloc x κ)))
...
(define (memo-size state)
  ...
  (let ((m (match state
                      ((ev _ _ _ _ _ _ m) m)
                      ((ko _ _ _ _ _ m) m))))
    (foldl + 0 (map set-count (hash-values m)))))
...
(define (memo-test)
  ...
  (define (answer-states states)
    (for/set ((s states) #:when (answer-state? s)) s))
  ...
  (for ((en ens))
    (let* ((e (eval en))
      ...
      (let* ((start (current-milliseconds))
        ...
        (printf "~a result ~a states ~a time ~a memo ~a size ~a memosize ~a\n" en
                (answer sys type-⊥ type-⊔) (set-count (system-states sys))
                duration memo-count
                (answer-size sys state-size) (answer-size sys memo-size))))))
