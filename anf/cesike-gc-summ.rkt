  ...
  (define (step)
      ...
      (let ((stacks (hash-ref Ξ τ #f)))
        (if stacks
            ...
            (begin
              ...
              (set! Ξi (add1 Ξi))))))
    (define (apply-local-kont ι κ v σ m)
      (match ι
        ((cons (letk x e ρ) ι)
         (let* ((a (alloc x #f))
                (ρ* (env-bind ρ x a))
    ...
    (define (gc s)
      (match s
        ((ev e ρ σ ι κ Ξ m)
         (let* ((ρ* (↓ ρ (free e)))
                (R (reachable (set-union (env-addresses ρ*) (apply set-union (set-map (stack-frames ι κ) touches))) σ))
                (σ* (↓ σ R)))
           (ev e ρ* σ* ι κ Ξ m)))
        ((ko ι κ v σ Ξ m)
    ...
    (lambda (s)
      (match s
        ((ev (? ae? ae) ρ σ ι κ _ m)
         (set (ko ι κ (eval-atom ae ρ σ) σ Ξi m)))
        ((ev `(if ,ae ,e1 ,e2) ρ σ ι κ _ m)
         (let ((v (eval-atom ae ρ σ)))
           (set-union (if (true? v)
                          (set (ev e1 ρ σ ι κ Ξi m))
                          (set))
                      (if (false? v)
                          (set (ev e2 ρ σ ι κ Ξi m))
                          (set)))))
        ((ev `(let ((,x ,e0)) ,e1) ρ σ ι κ _ m)
         (set (ev e0 ρ σ (cons (letk x e1 ρ) ι) κ Ξi m)))
        ((ev `(letrec ((,x ,e0)) ,e1) ρ σ ι κ _ m)
         (let* ((a (alloc x #f))
                (ρ* (env-bind ρ x a))
                (σ* (store-alloc σ a ⊥)))
           (set (ev e0 ρ* σ* (cons (letreck a e1 ρ*) ι) κ Ξi m))))
        ((ev `(set! ,x ,ae) ρ σ ι κ _ m)
         (let* ((v (eval-atom ae ρ σ))
                (a (env-lookup ρ x))
                (σ* (store-update σ a v)))
           (set (ko ι κ v σ* Ξi m))))
        ((ev `(quote ,e) ρ σ ι κ _ m)
         (match-let (((cons v σ) (alloc-literal e σ)))
           (set (ko ι κ v σ Ξi m))))
        ((ev (and `(,rator . ,rands) e) ρ σ ι κ _ m)
         (match-let (((ev _ Γρ Γσ _ _ _ _) (gc s)))
           (let ((v (eval-atom rator Γρ Γσ)))
               ...
               (if (null? rands)
                   (for/fold ((states (set))) ((w (γ v)))
                     (match w
                        ...
                        (let ((τ (ctx w rvs Γσ)))
                          (if (hash-has-key? m τ)
                              (let ((vσs (hash-ref m τ)))
                                (for/fold ((states states)) ((vσ vσs)) (MEMO "MEMO") (set-add states (ko ι κ (car vσ) (cdr vσ) Ξi m))))
                              (let loop ((x x) (vs (reverse rvs)) (ρ* ρ**) (σ* Γσ))
                                (match x
                                  ('()
                                   (stack-alloc! τ (cons ι κ))
                                   (set-union states (set (ev e0 ρ* σ* '() τ Ξi m))))
                                  ((cons x xs)
                                   (if (null? vs)
                                       (set)
                                       (let ((a (alloc x e)))
                       ...
                       ((prim _ proc)
                        (set-union states (list->set (set-map (proc e (reverse rvs) Γσ ι κ Ξi) (lambda (vσ) (ko ι κ (car vσ) (cdr vσ) Ξi m))))))
                       ((prim2 _ proc)
                        (set-union states (set (ko ι κ (apply proc (reverse rvs)) Γσ Ξi m))))
                       (_ (set))))
                   (let ((v (eval-atom (car rands) Γρ Γσ)))
         ...
         (let ((ικGs (stack-pop ι κ (set))))
           (for/set ((ικG ικGs))
             (let ((m* (for/fold ((m m)) ((τ (caddr ικG))) (memo-cache m τ v σ))))
               (apply-local-kont (car ικG) (cadr ικG) v σ m*))))))))
  (define (inject e)
    (let ((global* (append global
      ...
      (let loop ((global global*) (ρ (hash)) (σ (hash)))
        (match global
          ('()
           (ev e (↓ ρ (free e)) σ `(,(haltk)) #f (hash) (hash)))
          ((cons (cons x v) r)
