...
(define (make-machine global α γ ⊥ ⊔ alloc true? false? α-eq?)
  ...
  (define (touches d)
    (if (set? d)
        ...
        (match d
          ((ev _ ρ _ ι κ _ _) (set-union (env-addresses ρ) (apply set-union (set-map (stack-frames ι κ) touches))))
          ((ko ι κ v _ _ _) (set-union (apply set-union (set-map (stack-frames ι κ) touches)) (touches v)))
