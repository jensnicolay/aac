...
(define-struct ev (e ρ σ ι κ Ξ) #:transparent); #:methods gen:equal+hash ((define equal-proc
                                               ;                           (lambda (s1 s2 requal?)
                                                ;                            (and (equal? (ev-e s1) (ev-e s2))
                                                 ;                                (equal? (ev-ρ s1) (ev-ρ s2))
                                                  ;                               (equal? (ev-σ s1) (ev-σ s2))
                                                   ;                              (equal? (ev-ι s1) (ev-ι s2))
                                                    ;                             (equal? (ev-κ s1) (ev-κ s2))
                                                     ;                            (= (ev-Ξ s1) (ev-Ξ s2)))))
                                                      ;                  (define hash-proc (lambda (s rhash) (rhash (ev-e s))))
                                                       ;                 (define hash2-proc (lambda (s rhash) (rhash (ev-e s))))))
      ...
      (if (null? todo)
          ...
          (let ((s (car todo)))
            (if (set-member? visited s)
                (loop visited (cdr todo))
                (loop (set-add visited s) (append  (set->list (step s)) (cdr todo)))))))))
