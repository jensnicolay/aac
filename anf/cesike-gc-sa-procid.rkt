...
(define (make-step α γ ⊥ ⊔ alloc true? false? proc)
  ...
  (lambda (s)
    (match s
      ((ev (? ae? ae) ρ σ ι κ Ξ m r)
       (match-let (((vmr v m r) (eval-atom ae ρ σ (delay (contexts κ Ξ)) m r)))
         (set (ko ι κ v σ Ξ m r))))
      ((ev `(if ,ae ,e1 ,e2) ρ σ ι κ Ξ m r)
       (match-let (((vmr v m r) (eval-atom ae ρ σ (delay (contexts κ Ξ)) m r)))
         (set-union (if (true? v)
                        (set (ev e1 ρ σ ι κ Ξ m r))
                        (set))
                    (if (false? v)
                        (set (ev e2 ρ σ ι κ Ξ m r))
                        (set)))))
      ...
      ((ev `(,rator . ,rands) ρ σ ι κ Ξ m r)
       (let* ((RI (reachable (touches s) σ))
              ...
              (Γr (↓ r R))
              (Γm (↓ m I))
              (ctxs (delay (contexts κ Ξ))))
         (match-let (((vmr v m r) (eval-atom rator ρ Γσ ctxs Γm Γr)))
           (let loop ((rands rands) (rvs '()) (m m) (r r))
             (if (null? rands)
      ...
      ((ko (cons (haltk) _) _ v _ _ _ _)
       (set))
      ((ko ι κ v σ Ξ m r)
       (set (apply-local-kont ι κ v σ Ξ m r))))))
...
(define type-step (make-step type-α type-γ type-⊥ type-⊔ mono-alloc type-true? type-false? λ-proc))
