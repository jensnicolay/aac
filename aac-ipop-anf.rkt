...
; CESIK*Ξ machine for ANF Scheme (lambda if set! let letrec)
; guarded pop + administrative step when ι empty
; local Ξ
              ...
              (if (set-empty? todo)
                  ...
                  (let ((ικ (set-first todo)))
                    (match ικ
                      ...
                      (_ (loop (set-rest todo) (set-add result ικ) seen))))))))
         (ae?
          (lambda (e)
            (match e
              ((? symbol? e) #t)
                  ...
                  (match x
                    ('() 
                     (let* ((τ (ctx w rands σ))
                       ...
                       (set-union r (set (ev e0 ρ* σ* '() τ Ξ*)))))
                    ((cons x xs)
                     (if (null? rands)
                         (set)
                         (let ((a (alloc x)))
                           (loop xs (cdr rands) (env-bind ρ* x a) (store-alloc σ* a (eval-atom (car rands) ρ σ)))))))))
               ((? procedure? w)
                (let ((vs (map (lambda (ae) (eval-atom ae ρ σ)) rands)))
                  (set-union r (set (ko ι κ (apply w vs) σ Ξ)))))
               (_ (set))))))
        ((ko (cons (letk x e ρ) ι) κ v σ Ξ)
         (let* ((a (alloc x))
                (ρ* (env-bind ρ x a))
                (σ* (store-alloc σ a v)))
           (set (ev e ρ* σ* ι κ Ξ))))
        ((ko (cons (letreck a e ρ) ι) κ v σ Ξ)
         (let ((σ* (store-alloc σ a v)))
           (set (ev e ρ σ* ι κ Ξ))))
        ((ko (cons (haltk) _) _ v _ _)
         (set))
        ((ko '() τ v σ Ξ)
         (let ((ικs (stack-pop τ Ξ)))
           (list->set (set-map ικs (lambda (ικ)
                                      (ko (car ικ) (cdr ικ) v σ Ξ))))))))))
...
(time
 ...
 (conc-eval rotate)
 (type-eval rotate)
 ) ; cpu time: 217120 real time: 217000 gc time: 151
