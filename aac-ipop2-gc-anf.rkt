...
(define (make-step α γ ⊥ ⊔ alloc true? false?)
  ...
  (define (stack-frames ι κ Ξ)
    (let loop ((todo (set (cons ι κ))) (result (set ι)) (seen (set)))
      (if (set-empty? todo)
          result
       ...
       (let* ((R (reachable (touches s) σ))
         ...
         (let ((v (eval-atom rator ρ Γσ)))
           (for/fold ((r (set))) ((w (γ v)))
             (match w
               ((clo `(lambda ,x ,e0) ρ**)
                (let loop ((x x) (rands rands) (ρ* ρ**) (σ* Γσ))
                  (match x
                    ('() 
                     (let* ((τ (ctx w rands Γσ))
                    ...
                    ((cons x xs)
                     (if (null? rands)
                         (set)
                         (let ((a (alloc x)))
                           (loop xs (cdr rands) (env-bind ρ* x a) (store-alloc σ* a (eval-atom (car rands) ρ Γσ)))))))))
               ((? procedure? w)
                (let ((vs (map (lambda (ae) (eval-atom ae ρ Γσ)) rands)))
                  (set-union r (set (ko ι κ (apply w vs) Γσ Ξ)))))
               (_ (set)))))))
       ((ko '() κ v σ Ξ)
...
(time
 ...
 (conc-eval rotate)
 (type-eval rotate)
) ; cpu time: 35792 real time: 35786 gc time: 109
